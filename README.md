...
This is the sequence that avoids the issues people usually hit. Follow it in order —
most "it works locally but not in prod" problems come from skipping or reordering one
of these steps.

### 0. Prerequisites
- A VPS (2 vCPU / 4GB RAM minimum — DigitalOcean, Hetzner, Linode all work) with Docker
  and Docker Compose installed.
- A domain, with **two DNS A records** pointed at the server's IP before you start:
  `yourdomain.com` (frontend) and `api.yourdomain.com` (backend). Caddy (step 4) needs
  these resolving correctly to issue TLS certificates — if they're not propagated yet,
  wait before proceeding, or Caddy will fail to get a certificate and retry-loop.
- A **live** Paystack account with Transfers enabled (a separate manual approval from
  Paystack, not automatic) if you want payouts to work, not just challenge purchases.
- Your MT5 bridge service deployed and reachable (see the Phase 2/3 notes above — this
  is a separate service you build/license against your broker's MT5 Manager API; nothing
  in this repo provides it).
...
### 2. Create BOTH env files — this is the most commonly missed step
There are two separate env files with two separate purposes; missing either one causes
a different class of failure:

```bash
cp .env.example .env                                    # compose-level substitution (VITE_API_URL)
cp backend/.env.production.example backend/.env.production  # container runtime env (Laravel, DB, Paystack, etc.)
```

Edit **`.env`** (project root): set `VITE_API_URL` to `https://api.yourdomain.com/api/v1`.
This gets baked into the frontend at *build* time (Vite embeds it into the JS bundle) —
if you change it later, you must rebuild the frontend image, restarting the container
alone won't pick up the change.

Edit **`backend/.env.production`**: at minimum, set real values for `DB_PASSWORD` /
`POSTGRES_PASSWORD` (must match each other), `APP_URL`, `APP_FRONTEND_URL`,
`CORS_ALLOWED_ORIGINS` (exact scheme+domain the frontend is served from — a mismatch
here is the single most common "login works but nothing else does" bug, since the
browser silently drops the refresh_token cookie on a CORS mismatch), and your live
`PAYSTACK_PUBLIC_KEY`/`PAYSTACK_SECRET_KEY`. Leave `APP_KEY` and `JWT_SECRET` blank —
you'll generate those in step 5.

### 3. Point the domains at the right places
Edit `infra/Caddyfile` and replace the two placeholder domains
(`api.yourdomain.com`, `yourdomain.com`/`www.yourdomain.com`) with your real ones —
these must match what you set in `CORS_ALLOWED_ORIGINS` and `VITE_API_URL` above,
or requests will be rejected by CORS even though everything else is configured correctly.

### 4. Build and start everything
```bash
docker compose -f docker-compose.prod.yml up -d --build
```
Caddy will automatically request and renew TLS certificates for both domains on first
boot — check `docker compose -f docker-compose.prod.yml logs caddy` if a domain doesn't
come up over HTTPS within a minute or two (almost always a DNS propagation delay, not a
Caddy problem).

### 5. One-time application setup (run once, right after first boot)
```bash
...
```
Deliberately **not** running the full `DatabaseSeeder` in production — it also creates a
demo admin (`admin@propfirm.io` / `ChangeMe123!`) and (in local/testing envs only) fake
traders. Create your real admin account instead:
```bash
docker compose -f docker-compose.prod.yml exec backend php artisan tinker
>>> $admin = App\Models\User::create(['name' => 'Your Name', 'email' => 'you@yourdomain.com', 'password' => Hash::make('a-real-strong-password'), 'referral_code' => Str::upper(Str::random(8)), 'email_verified_at' => now(), 'kyc_status' => 'verified']);
...
`https://api.yourdomain.com/api/v1/webhooks/paystack`. Without this, payments will still
work via the redirect-polling fallback (`GET /checkout/{reference}` also independently
verifies), but payouts and any delayed confirmation rely on the webhook firing.

### 7. Verify it's actually working
```bash
curl https://api.yourdomain.com/up                  # Laravel health check — should return 200
curl https://yourdomain.com                          # frontend should load
```
Then do one real click-through: register an account, browse challenges, and confirm the
seeded pricing tiers show up. If the frontend loads but API calls fail, it's almost
always the `CORS_ALLOWED_ORIGINS` / `VITE_API_URL` mismatch described in step 2.
...
docker compose -f docker-compose.prod.yml exec backend php artisan migrate --force
```
The GitHub Actions workflow already builds and pushes images to GHCR on every push to
`main` — wiring a webhook or SSH step to run the two commands above automatically on
your server is the natural next step once you're comfortable with manual deploys.
